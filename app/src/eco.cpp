...
void ECOTracker::init(cv::Mat &im, const Rect2f &rect, const nlohmann::json &cfg_param)
{
	...
	// step 2. extract features from the first frame.
	xl = extractor(im, pos_, vector<float>(1, currentScaleFactor_), params_, is_color_image_);

	// step 3. process features using the cosine window.
    EcoFeats xlw;
	...
	// step 9. calculate sample energy and projection map energy.
	...
	// step 10. train the filter and update the projection matrix
	...
	// step 11. re-project feature and update the sample space.
    xlf_porj = projectFeature(xlf, projection_matrix_);
...
bool ECOTracker::update(const cv::Mat &frame, Rect2f &roi) {
	...
	// step 6. compute the scores and optimize continuous score function with Newton's method.
	...
	for (size_t i = 0; i < xtf_proj.size(); i++) {
		...
		for (size_t j = 0; j < xtf_proj[i].size(); j++)	{
			size_t k1 = j / hf_full_[i].size();
			size_t k2 = j % hf_full_[i].size();
    ...
    // step 7. update sample space
	// step 7-1. get the sample calculated in localization
	EcoFeats xlf_proj;
	for (size_t i = 0; i < xtf_proj.size(); ++i) {
	...
	// step 7-2. shift the sample so that the target is centered,
	...
	// step 7-3. update the samples space to include the new sample, the distance matrix,
	sample_update_.updateSampleSpaceModel(xlf_proj);

	// step 8. train the filter every Nsth frame
	...
	// step 9. update filter f and tracking target
	hf_full_ = fullFourierCoeff(hf_);
